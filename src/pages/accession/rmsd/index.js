...
const rawTextToData = raw => {
  ...
  const data = {
    ...
    rmsd: new Float32Array(length),
  };
  for (const [index, line] of lines.entries()) {
    const [time, rmsd] = line
      .split(MULTIPLE_SPACES_REGEXP)
      .map(Number.parseFloat);
    data.time[index] = time;
    data.rmsd[index] = rmsd;
  }
  return { data };
};
...
class _RMSd extends PureComponent {
  ...
  #draw = hovered => {
    ...
    //   .selectAll('path.rmsd-data-avg')
    //   .data(yValues.map(d => d[0]));
    // linesAvg
    //   .enter()
    //   .append('path')
    //   .attr('class', ([key]) => `rmsd-data-avg ${key}`)
    //   .attr('fill', 'none')
    //   .attr('stroke', d => colors[d])
    //   .attr('opacity', 0.5)
    //   .attr('stroke-linejoin', 'round')
  ...
  async componentDidMount() {
    ...
    const response = await fetch(
      BASE_PATH + accession + '/md.rmsd.vs1stSnp.xvg',
    );
